...
tools/                  — ручные проверки на живом репозитории
  layout-audit.js       — аудит вёрстки в браузере: переполнение, обрезка, наложения
...
- Мобильная вёрстка обязательна: проверять на 375px. Наложения кнопок, вылезание текста
  за экран и переполнение своей коробки проверяются не глазами, а `tools/layout-audit.js`:
  запустить в браузере на экране и посмотреть на нули в `counts`.
- **Палитра — одна.** Источник правды для цветов, радиусов и теней — блок токенов в
  `design.css`; контраст пар проверяется `tests/contrast.test.mjs` (4.5:1 для текста,
  3:1 для границ). Новый цвет без пары в этом тесте — незамеченный нечитаемый текст.
